...
export default function BaristasIndex() {
  const { baristas } = useLoaderData<typeof loader>()

  return (
    <div className='flex flex-col gap-4 max-w-[430px] m-6'>
      {baristas.map((barista) => (
        <Link key={barista.id} to={`/baristas/${barista.id}`} className='border p-4 space-y-4'>
          <div className='grid grid-cols-5 gap-6 items-center'>
            <div className='size-14 bg-gray-100 rounded-full flex items-center justify-center text-gray-300 col-span-1'>
              <UserRound />
            </div>
            <div className='col-span-3'>
              <p className='text-lg'>{barista.profile?.name}</p>
